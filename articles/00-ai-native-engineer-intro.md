...
MacBook Air M1（16GB）一台で、Rust と Julia を限界までチューニングしながらオンデバイスAIを独力開発しています。週20本超の arXiv 論文を実装する生活。鬱と不眠をハイオク燃料にして駆動中です。
...
すべて独学です。コーチはいません。論文を読み、データを取り、仮説を立てて検証する。このプロセスは、今のAI研究のやり方とほとんど同じです。学校生活も評定平均4.5、無遅刻無欠席。書道で県特選。やれることは全部やりました。
...
「Python + PyTorch でいいじゃん」という声が聞こえそうなので、もう少し踏み込みます。Python が向かない理由は速度だけではありません。Python のDLフレームワークは「Define-and-Run」か「Define-by-Run」かを問わず、計算グラフという中間表現を経由します。この中間表現が言語の型システムと分離しているため、テンソルの形状不整合やデバイス不整合が実行時まで検出できません。Julia では言語の型システムがそのまま計算の型システムになるので、`@code_warntype` でコンパイル前に型の問題を洗い出せます。研究のイテレーションで一番時間を食うのは「動かしてみたらエラー」のデバッグなので、これが消えるだけでも生産性が段違いです。

- **DL**: `Lux.jl` + `Reactant.jl`（MLIR/XLA バックエンド）で次世代の深層学習パイプラインを組んでいます。Lux を Flux ではなくメインに据えているのは、**明示的な状態管理**のためです。Flux はモデルの中にパラメータを暗黙的に保持しますが、Lux はパラメータとモデル構造を完全に分離します。関数型スタイルで `model(x, params, state)` と書けるので、パラメータの受け渡しが透明になり、カスタム学習ループや分散学習との相性が良い。`Flux.jl` + `Zygote.jl` も自動微分の研究で併用していますが、新しいモデルを書くときは Lux が第一選択です
- **微分方程式**: `DifferentialEquations.jl`。Julia 発のこのエコシステムは他言語に代替がありません。Python の `torchdiffeq` や `diffrax`（JAX）も Neural ODE を扱えますが、ソルバの数と品質で Julia が圧倒的です。剛性問題への適応的ソルバ、確率微分方程式、遅延微分方程式まで、一つのパッケージで統一的に扱えます。微分方程式と深層学習の交差点にいる研究では、これが決定的でした
- **GPU**: `CUDA.jl` で `cu()` 一発のカーネル自動生成。`AcceleratedKernels.jl` でバックエンド非依存に。Metal / ROCm / oneAPI にも同じコードで対応できます。手書き CUDA C に比べて性能は 90-95% 程度ですが、開発速度は10倍以上。研究フェーズではこのトレードオフが正解です
...
「C/C++ でよくない？」という疑問に答えておきます。C は安全性の保証がなく、メモリバグは自己責任です。一人開発でメモリ破壊のデバッグに時間を溶かす余裕はありません。C++ は安全に書こうとすると、スマートポインタやムーブセマンティクスのルールを「人間が正しく運用する」必要があります。Rust は同じ安全性をコンパイラが強制してくれます。Go も候補でしたが、GC がある時点で Julia と同じ問題を抱えます。リアルタイム推論のレイテンシ保証には GC なしの言語が必要でした。

Rust を選んだもう一つの理由は、**C-ABI ハブとしての役割**です。Rust で書いたカーネルは `extern "C"` をつけるだけで、Julia からも Elixir からも Swift からも呼べます。一つのカーネルに複数の顔を持たせる——これが多言語アーキテクチャの要です。C でも同じことはできますが、Rust なら FFI 境界の外側（カーネル本体）をメモリ安全に保てます。

- **推論**: `Burn`（本番）/ `Candle` + `llama-cpp-rs`（LLM/GGUF）でエッジ推論。Burn を選んだのは、バックエンドの差し替えが可能だからです。`tract`（ONNX Runtime の Rust バインディング）や `ort` も検討しましたが、ONNX 形式に変換するステップが増えるのと、推論パイプラインのカスタマイズ性で Burn に軍配が上がりました。M1 の 16GB でも SLM なら十分回ります
- **Web/Backend**: `axum` + `tokio` + `sqlx` で型安全な非同期API。`actix-web` も高速ですが、axum は `tokio` エコシステムとの統合がシームレスで、`tower` ミドルウェアをそのまま使えます。DB は ORM（Diesel / SeaORM）ではなく `sqlx`——SQL をそのまま書きつつコンパイル時にスキーマ照合で型チェックできます。`wasm-bindgen` + `wgpu` でブラウザにもネイティブ並みでデプロイ
- **Data**: `polars` で ETL。`DataFusion` も検討しましたが、polars はメモリ上の DataFrame 操作に特化しており、ETL パイプラインでは API の簡潔さで勝ります。`bincode` + `zstd` で高圧縮シリアライズ。JSON ではなくバイナリフォーマットを選ぶのは、内部通信に人間可読性が不要だからです。Protocol Buffers も検討しましたが、スキーマ定義ファイルの管理コストが一人開発では割に合いません
...
「Go + goroutine で十分では？」と思うかもしれません。Go は並行処理に強いですが、障害回復はアプリケーション側で実装する必要があります。Erlang/Elixir の OTP は Supervisor ツリーという階層的な障害回復メカニズムをランタイムに組み込んでいます。プロセスが死んだときの再起動戦略（one_for_one、one_for_all、rest_for_one）を宣言的に定義するだけで、障害回復のコードを一行も書かずに耐障害性が手に入ります。Kubernetes のヘルスチェック + 再起動も似た発想ですが、K8s はコンテナ単位の粗い粒度です。OTP はプロセス単位——つまり一つの接続、一つのタスクという細かい粒度で障害を隔離できます。ソロ開発者にとっては、この粒度の細かさが夜の安眠に直結します。

- `GenServer` / `Supervisor` で自己修復プロセスツリー
- `Phoenix LiveView` でリアルタイムUI。SPA + API の構成にしなかった理由は、フロントとバックの二重メンテナンスを一人で回す余裕がないからです。LiveView ならサーバーサイドで状態を管理し、差分だけクライアントに送ります。JavaScript をほぼ書かずにリアルタイムUIが手に入る
- `rustler` で Rust NIF を BEAM 上に展開。重い計算は Rust に逃がしつつ、耐障害性は BEAM が担保

### Skin: Swift / Kotlin — ネイティブ体験

各OSのデザイン言語を尊重します。クロスプラットフォームではなく、ネイティブ。

React Native や Flutter を使わない理由は明確で、オンデバイス推論のパフォーマンスとOS統合の深さが必要だからです。`Core ML` や `NNAPI` を直接叩くには、ネイティブの言語で書くしかありません。
...
知識の蓄積は Obsidian に集約しています。Notion ではなく Obsidian を選んだのは、**データがすべてローカルの Markdown ファイル**だからです。Notion はクラウドに依存し、エクスポートすると書式が崩れます。Obsidian ならファイルシステム上のプレーンテキストなので、バージョン管理でき、他のツールからも直接読み書きできます。ローカル SLM で RAG 検索する際にも、Markdown ファイルをそのまま読み込めるのが決定的でした。
...
なぜ GitHub Copilot ではなく Claude Code + Cursor か。Copilot は「次の行を予測する」補完ツールですが、Claude Code は「設計を渡して実装全体を任せる」エージェントです。一人でチーム開発を回すには、行単位の補完ではなく、ファイル単位・機能単位で委譲できるエージェントが必要でした。Cursor はエディタ統合の完成度が高く、コードベース全体をコンテキストとして渡せるので、大規模なリファクタリングでもコンテキストが途切れません。

具体的には、要件を自然言語で書き、アーキテクチャを設計し、AIエージェントに実装を委譲します。出力されたコードをレビューし、設計意図とのズレを修正する。このサイクルを高速に回すことで、一人でもチーム開発に近いスループットを出せています。「コードを書く」のではなく「コードを設計する」のが仕事です。

### Infra

Hetzner（CPX11, 月€4.99）+ Cloudflare + Coolify で格安運用しています。月約750円でプロダクションレベルのインフラが組めます。

なぜ AWS / GCP / Vercel ではなく Hetzner か。理由は単純で、**月額コストが桁違い**です。AWS の最小構成でも月$10-20 はかかりますが、Hetzner なら€4.99 で 2vCPU / 2GB RAM / 40GB NVMe のサーバーが借りられます。Cloudflare の無料プランで CDN / DNS / DDoS 対策を被せ、Coolify（セルフホスト型の PaaS）でデプロイを自動化すれば、Vercel 相当の DX が月約750円で手に入ります。Coolify を選んだのは Docker Compose をそのまま使えるからで、ベンダーロックインがありません。

監視は `tracing` + OpenTelemetry で分散トレーシングです。`log` クレートではなく `tracing` を選んだのは、スパン（処理の区間）を構造的に表現できるから。`log` はポイントの記録、`tracing` は区間の記録。「このリクエストがどのサービスを通って何 ms かかったか」を追跡できます。RED metrics（Rate / Errors / Duration）を計測し、異常を自動検知。一人運用だからこそ監視は手を抜けません。
